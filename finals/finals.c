...
float total_grade(float a, float b, float c, float d, float e) 
{
...
void output_grade (float a){
    printf("\nFullname: %s  %s\n", s1.first_name, s1.last_name);
    printf("Age: %d\n", s1.age);
    printf("\nYour desired grade is %.2f\n", s1.grade);
    s1.final_grade = a;
    printf("Your Grade based on your computation is %.2f\n\n", s1.final_grade);
    if (s1.grade >= s1.final_grade){
...
// Fix them.
void save_to_file(struct student *s) {
...
int main() {
    ...
    do {
        ...
        float a_grade = total_grade(a,b,c,d,e);
        output_grade(a_grade);
